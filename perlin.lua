...
--   h = hash % 16                      -- CONVERT LO 4 BITS OF HASH CODE
--   u = nil
--   v = nil
-- 
--   if h<8 then u=x else u=y end       -- INTO 12 GRADIENT DIRECTIONS.
--   if h<4                   then v=y
--   elseif h >=12 and h <=14 then v=x
--   else                          v=z
--   end
-- 
...
--   X = math.floor(x) % 256                  -- FIND UNIT CUBE THAT
--   Y = math.floor(y) % 256                  -- CONTAINS POINT.
--   Z = math.floor(z) % 256
...
--   w = fade(z)
--   A = p[X+1]+Y; AA = p[A+1]+Z; AB = p[A+2]+Z      -- HASH COORDINATES OF
--   B = p[X+2]+Y; BA = p[B+1]+Z; BB = p[B+2]+Z      -- THE 8 CUBE CORNERS,
-- 
